...
const WidthdrawLimit = () => {
    ...
    const onSubmit = data => {
        axios.post('http://localhost:5000/api/option/set', data)
            .then((res) => {
                console.log('WidthdrawLimit', res);
                navigate('/admin/settings');
            })
            .catch((err) => {
                console.log('this is WidthdrawLimit error', err);
            })
    }

    useEffect(() => {
        axios.get('http://localhost:5000/api/option/get/widthdrawLimit')
            .then((res) => {
                console.log('WidthdrawLimit', res);
            ...
            <div className='py-2'>
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="px-4">
                        ...
                        <input {...register("key")} type="hidden" defaultValue='widthdrawLimit' name="key" />
                    </div>
                    <div className="px-4">
